...
app.post("/account/api/oauth/token", async (req, res) => {
    ...
    if (req.user.banned) {
        if (req.user.banExpires && new Date(req.user.banExpires).getTime() <= new Date().getTime()) {
            await User.updateOne({ accountId: req.user.accountId }, { $set: { banned: false, banExpires: null, banReason: null } });
        } else {
            let banMessage = `You have been permanently banned: ${req.user.banReason || "No reason provided"}`;
            if (req.user.banExpires) {
                const timeLeft = Math.round((new Date(req.user.banExpires).getTime() - new Date().getTime()) / 60000);
                banMessage = `You are temporarily banned for: ${req.user.banReason || "No reason provided"}. Expires in ${timeLeft} minutes.`;
            }
