...
async function registerUser(discordId, username, email, plainPassword) {
    ...
    try {
        await User.create({
            ...
            await updateCosmeticCount(i.accountId);
        });
...
async function updateCosmeticCount(accountId) {
    ...
    for (const key in athena.items) {
        ...
        if (item.templateId && item.templateId.startsWith("AthenaCharacter:")) skins++;
        if (item.templateId && item.templateId.startsWith("AthenaPickaxe:")) pickaxes++;
    }

    await User.updateOne({ accountId }, { $set: { skins, pickaxes } });
}
